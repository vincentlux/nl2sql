...
class SQLNetCondPredictor(nn.Module):
    ...
    predict number of cols
    predict each column name
    predict operator
    predict value (substring)
    '''
    ...
    def forward(self, x_emb_var, x_len, col_inp_var, col_name_len,
        ...
        B = len(x_len)
        if reinforce:
            raise NotImplementedError('Do not have RL')
        
        # Predict the number of conditions
        # First use column embeddings to calculate the initial hidden input
        # Then run lstm and predict condition number
